...
function insertIntoTable(tbl, newItems)
    for _, item in ipairs(newItems) do
        table.insert(tbl, item)
    end
end

if SandboxVars.MoreSmokes.YesTobacco then
    local tobaccoPack = {"MSCigarettePack", "CigarilloPack", "CigarBox"}
    local tobaccoSeed = {"SeedPackTobacco"}
    local tobaccoTobacco  = {"TobaccoZipLockBag", "ChewingTobaccoCan"}
    local tobaccoPipe = {"SmokePipe1", "SmokePipe2", "SmokePipe3", "SmokePipeGlassBlue", "SmokePipeGlassGreen", "SmokePipeGlassYellow", "SmokePipePink"}
    local tobaccoHookah  = {"Hookah1", "Hookah2", "Hookah3"}
    local tobaccoRecipe = {"MoreSmokesMagazine1", "MoreSmokesMagazine2"}

    insertIntoTable(MoreSmokes.options.Pack, tobaccoPack)
    insertIntoTable(MoreSmokes.options.Seed, tobaccoSeed)
    insertIntoTable(MoreSmokes.options.Tobacco, tobaccoTobacco)
    insertIntoTable(MoreSmokes.options.Pipe, tobaccoPipe)
    insertIntoTable(MoreSmokes.options.Hookah, tobaccoHookah)
    insertIntoTable(MoreSmokes.options.Recipe, tobaccoRecipe)
end

if SandboxVars.MoreSmokes.YesCannabis then
    local cannabisPack = {"JointsPackNorthernLights", "JointsPackPurpleHaze", "JointsPackSourDiesel", "BluntsPackNorthernLights", "BluntsPackPurpleHaze", "BluntsPackSourDiesel", "SpliffsPackNorthernLights", "SpliffsPackPurpleHaze", "SpliffsPackSourDiesel" }
    local cannabisSeed = {"SeedPackNorthernLights", "SeedPackPurpleHaze", "SeedPackSourDiesel"}
    local cannabisCannabis = {"NorthernLightsZipLockBag", "PurpleHazeZipLockBag","SourDieselZipLockBag"}
    local cannabisBong = {"Bong1v1", "Bong1v2", "Bong1v3", "Bong1v4", "Bong2v1", "Bong2v2", "Bong2v3", "Bong2v4", "Bong3v1", "Bong3v2", "Bong3v3", "Bong3v4", "Bong4v1", "Bong4v2", "Bong4v3", "Bong4v4", "Bong5"}
    local cannabisEdibles = {"CannabisButter", "CannabisChocolateBar", "CannabisBrownie", "CannabisGummies"}
    local cannabisRecipe = {"MoreSmokesMagazine1", "MoreSmokesMagazine3", "MoreSmokesMagazine4"}

    insertIntoTable(MoreSmokes.options.Pack, cannabisPack)
    insertIntoTable(MoreSmokes.options.Seed, cannabisSeed)
    insertIntoTable(MoreSmokes.options.Cannabis, cannabisCannabis)
    insertIntoTable(MoreSmokes.options.Bong, cannabisBong)
    insertIntoTable(MoreSmokes.options.Edibles, cannabisEdibles)
    insertIntoTable(MoreSmokes.options.Recipe, cannabisRecipe)
end
